...
class ServerConnection {
    ...
    _endpoint() {
        ...
        //  return url;
        //since there is no 'location.' in Electron, this must be done in order to connect (cant connect to 'ws:///server/webrtc' obviusly)
        return 'wss://snapdrop.net/server/webrtc'
    }
...
class PeersManager {
    ...
    constructor(serverConnection) {
        ...
        ipcRenderer.on('sendFiles', (event, e) => {     //called by main.js ln.181
            ...
            for (let i = 0; i < e.data.length; i++){    //loop though enterys
                message.files[i] = new File(
                    ...
                    e.name[i],                          //Filename 
                    {type: e.type[i]}                   //MIME-type, since FileExtention isnt enought for iOS 
                );
                //message.files[i].path = e.paths[i]; //cant set path like this --> Find a workaround
            };
