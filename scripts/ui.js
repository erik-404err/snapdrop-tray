//modiefied/added ln. 16, 32, 45, 56, 417, 534
...
Events.on('display-name', e => {
    ...
    ipcRenderer.send('sendName', me.displayName);   //send name to main.js ln.25
});
...
class Notifications {
    ...
    _notify(message, body, closeTimeout = 20000) {
        const config = {
            body: body,
            icon: './images/logo_transparent_512x512.png', //added '.' before '/' since the image couldn't load otherwise
        }
        let notification;
        try {
...
if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('./service-worker.js') //added '.' before '/' because the script can't be found otherwise
        .then(serviceWorker => {
